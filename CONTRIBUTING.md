...
HighSign is a sideloading app meant to be used on stock versions, to keep compatibility we have to utililize stock features to keep it working. As such, we have specific contribution rules in place to maintain this and its integrity.
...
- **If you're planning on making a large contribution, please [make an issue](https://github.com/okphong582-png/hb/issues) beforehand**.
...
1. Clone the repository
    ```sh
    git clone https://github.com/okphong582-png/hb --recursive
    ```
    - `Zsign` is a submodule, recursive is required.

2. Opening with Xcode
    ```sh
    cd HighSign && open HighSign.xcodeproj
    ```

#### Localizations
- For localizations, you will need Xcode 15 or higher to edit the `HighSign/Resources/Localizable.xcstrings` file, or some alternative software which allows you to edit an `.xcstrings` file. 
