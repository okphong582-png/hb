...
struct FilesView: View {
    ...
    private var addButton: some View {
        Menu {
            ...
            Divider()
            Button {
                let freed = StorageCleanerManager.shared.performQuickClean()
                viewModel.loadFiles()
                UIAlertController.showAlertWithOk(
                    title: .localized("Dọn dẹp tối ưu máy"),
                    message: .localized("Đã dọn dẹp thành công! Đã giải phóng \(freed) bộ nhớ tạm để tối ưu máy.")
                )
            } label: {
                Label(String(localized: "Dọn dẹp file tạm (Tối ưu máy)"), systemImage: "bolt.badge.checkmark.fill")
            }
            Button(role: .destructive) {
                UIAlertController.showAlertWithCancel(
                    title: .localized("Dọn dẹp toàn bộ dữ liệu"),
                    message: .localized("Hành động này sẽ xoá toàn bộ dữ liệu file đã tải và file tạm để tối ưu máy nhất có thể. Bạn có chắc chắn?"),
                    actions: [
                        UIAlertAction(title: .localized("Xoá tất cả"), style: .destructive) { _ in
                            let freed = StorageCleanerManager.shared.performDeepClean()
                            viewModel.loadFiles()
                            UIAlertController.showAlertWithOk(
                                title: .localized("Hoàn tất"),
                                message: .localized("Đã giải phóng \(freed) dung lượng máy.")
                            )
                        }
                    ]
                )
            } label: {
                Label(String(localized: "Xoá all dữ liệu file (Tối ưu nhất)"), systemImage: "trash.circle.fill")
            }
        } label: {
