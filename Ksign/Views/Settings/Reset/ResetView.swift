...
struct ResetView: View {
	@StateObject private var cleaner = StorageCleanerManager.shared
    ...
    var body: some View {
		...
		.onAppear {
			cleaner.refreshStorageInfo()
		}
		.alert(cleanResultMessage ?? "", isPresented: $showCleanAlert) {
...
extension ResetView {
	@ViewBuilder
	private func _optimizer() -> some View {
		NBSection("Tối ưu hoá & Dọn dẹp máy (HighSign)") {
			HStack {
				Label("Dung lượng chiếm dụng", systemImage: "internaldrive.fill")
				Spacer()
				Text(cleaner.totalDiskUsage)
					.bold()
					.foregroundStyle(.accent)
			}
			
			Toggle("Tự động dọn dẹp sau khi cài đặt", isOn: $cleaner.autoCleanOnInstall)
			
			Button {
				let freed = cleaner.performQuickClean()
				cleanResultMessage = "Đã dọn dẹp thành công! Giải phóng \(freed) bộ nhớ tạm để máy hoạt động mượt mà nhất."
				showCleanAlert = true
			} label: {
				Label("Dọn dẹp nhanh (File tạm & Cache)", systemImage: "bolt.badge.checkmark.fill")
			}
			.tint(.accent)
			
			Button(role: .destructive) {
				Self.resetAlert(
					title: "Dọn dẹp toàn bộ dữ liệu máy",
					message: "Hành động này sẽ xóa toàn bộ IPA đã tải, file giải nén và bộ nhớ tạm để giải phóng 100% dung lượng máy."
				) {
					let freed = cleaner.performDeepClean()
					cleanResultMessage = "Đã xóa toàn bộ dữ liệu & file tạm thành công! Đã giải phóng \(freed) dung lượng máy."
					showCleanAlert = true
				}
			} label: {
				Label("Dọn sạch toàn bộ dữ liệu & File lưu", systemImage: "trash.circle.fill")
			}
		} footer: {
			Text("Tính năng tối ưu máy độc quyền của HighSign giúp máy bạn luôn sạch sẽ, không bị đầy bộ nhớ sau khi cài và ký ứng dụng.")
		}
	}
