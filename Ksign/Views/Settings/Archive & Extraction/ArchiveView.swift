...
struct ArchiveView: View {
	@AppStorage("Feather.compressionLevel") private var _compressionLevel: Int = 1
    ...
    var body: some View {
		NBList(.localized("Archive & Extraction")) {
			Section {
				Picker(.localized("Compression Level"), systemImage: "bolt.fill", selection: $_compressionLevel) {
					ForEach(ZipCompression.allCases.indices, id: \.self) { index in
						Text(ZipCompression.allCases[index].label).tag(index)
					}
				}
			} footer: {
				Text(.localized("Chế độ ký siêu nhanh giúp HighSign hoàn tất ký và đóng gói IPA chỉ trong 1-2 giây."))
			}
            ...
            Section {
                ...
                Text(.localized("Whether to remember the last location where a file was copied/moved to or use HighSign's documents folder as default."))
            }
