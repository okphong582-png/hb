...
final class ArchiveHandler: NSObject {
	...
	static func getCompressionLevel() -> Int {
		if UserDefaults.standard.object(forKey: "Feather.compressionLevel") == nil {
			return 1 // BestSpeed for Ultra-Fast HighSign
		}
		let level = UserDefaults.standard.integer(forKey: "Feather.compressionLevel")
		return min(max(0, level), ZipCompression.allCases.count - 1)
	}
}
